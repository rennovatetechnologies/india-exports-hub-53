...
import {
  ...
  Search, Filter, ArrowUpRight, Crown, Settings2, BarChart3, AlertTriangle,
} from "lucide-react";
...
export default function SuperAdminPage() {
  ...
  const filtered = useMemo(() => {
    return requests.filter((r) => {
      ...
      if (query && !`${r.name} ${r.email} ${r.department}`.toLowerCase().includes(query.toLowerCase())) return false;
      return true;
    });
      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <span className="inline-flex items-center gap-2 rounded-full glass px-3 py-1 text-[10px] uppercase tracking-[0.2em] text-white/70">
            <Crown size={11} className="text-[var(--gold)]" /> Super Admin
          </span>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 text-[11px]">
                      <span className="rounded-full border border-white/10 bg-white/[0.04] px-2 py-0.5 text-white/65 capitalize">
                        {r.role === ROLES.SUPER ? "Super admin" : "Operations"}
                      </span>
          ...
          <>
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-2 py-0.5 text-[10px] uppercase tracking-wider text-white/60">
                    {active.id} · {active.role === ROLES.SUPER ? "Super admin" : "Operations"}
                  </span>
                ...
                <button onClick={() => setActive(null)} className="rounded-md p-1 text-white/45 hover:text-white">✕</button>
              </div>
              ...
              <dl className="mt-5 grid grid-cols-2 gap-3 text-xs">
                ...
                <Info label="Requested role" value={active.role === ROLES.SUPER ? "Super admin" : "Operations"} />
              </dl>
              ...
              <div className="mt-5">
                ...
                <div className="mt-2 grid gap-2">
                  {["Cases · read/write", "KYC · approve", "Pricing · view", "Audit log · read"].map((p) => (
              ...
              <div className="mt-auto pt-6 grid grid-cols-2 gap-2">
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-gradient-to-r from-emerald-300 to-cyan-300 px-4 py-2.5 text-sm font-semibold text-black hover:opacity-90"
                >
